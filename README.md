...
<!-- Added by: pwt, at: Fri Jan  1 15:07:23 GMT 2021 -->
...
SoCo-CLI establishes a temporary local HTTP server from which Sonos can access the specified audio file, and instructs the speaker to play it. The action will terminate when playback has concluded. Example:
...
This feature works by invoking the `play_file` action for each file in the playlist in sequence, so the same rules apply as for `play_file`. Note that this does not create a Sonos queue on the speaker -- all queuing is handled locally.
