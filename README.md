...
- **`queue_multiple_search_results <search_result_numbers>`** (or **`qmsr`**): Queue one or more items from the last search performed, at the end of the queue. Search result numbers start from 1, and can be supplied as single integers, sequences (e.g., '4,7,3'), ranges (e.g., '5-10'), or 'all' (for all tracks). Note: do not use spaces either side of the commas and dashes. Sequences and ranges can be mixed, e.g., '1,3-6,10'.
